...
if (
    ...
    or (time.time() - os.path.getmtime(file_name)) / 3600 > 24 * 7
):
    try:
        ...
        print("Error while retriving database from MITRE Github", e)

try:
    Mitre_DB = pd.read_json(file_name)
    Mitre_DB = Mitre_DB[Mitre_DB["type"] == "attack-pattern"]
    Mitre_DB["mitre_id"] = Mitre_DB["external_references"].apply(
