...
class domainParser(ParserInterface):
    r"""Parse and extract domain names from text.

    Domain names with TLD validation against IANA's official TLD list.
    ...
    Example:
        >>> parser = domainParser("No domain here")
        >>> parser.contains()
        False
        >>> parser = domainParser("Visit example.com or google[.]com")
        >>> parser.contains()
        True
        >>> parser.extract()
    ...
    def extract(self) -> list[str]:
        """Extract all domain name instances from the text.

        Defanged notation (e.g., example[.]com) is normalized to standard format.
        Only domains with valid TLDs from IANA's list are returned.

        Returns:
            list[str]: A list of extracted domain name values.
        """
