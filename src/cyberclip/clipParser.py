...
try:
    from cyberclip import userAction, userTypeParser
...
class clipParser:
    ...
    def __init__(self, include="", exclude=""):
        ...
        log_file = os.getenv("LOG_FILE", str(Path(__file__).parent / "log.log"))
        self.log.handlers = [
            RotatingFileHandler(
                filename=log_file,
    ...
    def load_all(self):
        ...
        self.privateParserModuleImport = self.moduleLoader(
            userTypeParser.private, "userTypeParser.private"
        )
        self.ActionModuleImport = self.moduleLoader(userAction, "userAction")
        self.privateActionModuleImport = self.moduleLoader(
            userAction.private, "userAction.private"
        )

        # Load all parsers
        parserModules = dict(
            (name, m) for name, m in getmembers(self.parserModuleImport, ismodule)
        )
        privateParserModules = dict(
            (name, m)
            for name, m in getmembers(self.privateParserModuleImport, ismodule)
        )
        ...
        for parserModule in parserModules.keys():
            classes = dict(
                (name, c)
                for name, c in getmembers(parserModules[parserModule], isclass)
            )
        ...
        # Load all actions
        actionModules = dict(
            (name, m) for name, m in getmembers(self.ActionModuleImport, ismodule)
        )
        privateActionModules = dict(
            (name, m)
            for name, m in getmembers(self.privateActionModuleImport, ismodule)
        )
        ...
        for actionModule in actionModules.keys():
            classes = dict(
                (name, c)
                for name, c in getmembers(actionModules[actionModule], isclass)
            )
            for className in classes.keys():
                if "Action" in className and "ActionInterface" not in className:
                    instance = self.create_instance(
                        classes, className, parsers=self.parsers
                    )
    ...
    def parseData(self, data):
        ...
        if data:
            ...
            for action in self.actions.values():
                ...
                for parsertype, parser in self.parsers.items():
                    if (
                        parsertype in action.supportedType
                        and parsertype in self.detectedType
                    ):
        ...
        self.results = {
            ...
            "detectedType": self.detectedType,
    ...
    async def apply_actionable(self, actionable, text, complex_param={}) -> str:
        ...
        if "Action" in actionable.__module__:
            if action := self.actions.get(actionable.description):
                ...
                parsers_str = ",".join(
                    [str(parser.__class__) for parser in actionable.parsers.values()]
                )
