...
try:
    ...
    from cyberclip.tui.ActionPannel import ActionCommands, ActionPannel
    ...
    from cyberclip.tui.RecipesPannel import Recipe, RecipeButton, RecipesPannel
    ...
    from tui.ActionPannel import ActionCommands, ActionPannel
    ...
    from tui.RecipesPannel import Recipe, RecipeButton, RecipesPannel
...
class ClipBrowser(App):
    ...
    try:
        ...
        if not file.exists():
            file.write_text("[]")
        with open(
            Path(__file__).parent / "data/recipes.yml", "r", encoding="utf8"
        ) as f:
    ...
    async def on_mount(self):
        ...
        self.notify(
            f"Loaded parsers: {len(self.parser.parsers.keys())}\nLoaded actions: {len(self.parser.actions.keys())}"
        )
    ...
    async def get_param(self, actionnable: actionInterface | ParserInterface):
        if isinstance(actionnable, ParserInterface) or not actionnable.complex_param:
            ...
            param_screen.border_title = (
                f"Parameters for '{actionnable.description}' action."
            )
            param_screen.actionnable = actionnable
            try:
                self.push_screen(
                    param_screen, callback=partial(self.handle_param, actionnable)
                )
            except:
                self.notify(
                    f"Error while executing action : {traceback.format_exc()}",
                    severity="error",
                )

    async def handle_param(
        self,
        actionnable: actionInterface | ParserInterface,
        complex_param: dict | None = None,
        recipe_parser: bool = False,
    ):
        if complex_param:
        ...
        if self.active_tab == ClipBrowser.APP_TITLE:
            try:
                self.text = await self.parser.apply_actionable(
                    actionnable, self.query_one(TextArea).text
                )
                ...
                self.notify(
                    "Error applying action..." + str(e) + traceback.format_exc(),
                    severity="error",
                )
        elif self.active_tab == "Table View":
            dataframe = self.query_one(FiltrableDataFrame)
            column_name = dataframe.datatable.df.columns[
                dataframe.datatable.cursor_column
            ]
            new_column_name = dataframe.datatable.get_column_name(
                f"{actionnable.__class__.__name__}_{column_name}"
            )
            try:
                if recipe_parser:
                    dataframe.datatable.df[new_column_name] = await asyncio.gather(
                        *(
                            self.recipe_parser.apply_actionable(actionnable, str(text))
                            for text in dataframe.datatable.df[column_name]
                        )
                    )
                    dataframe.datatable.df[new_column_name] = (
                        dataframe.datatable.df.apply(
                            lambda x: clean_tsv(x[new_column_name], x[column_name]),
                            axis=1,
                        )
                    )
                    ...
                    dataframe.datatable.df[new_column_name] = await asyncio.gather(
                        *(
                            self.parser.apply_actionable(actionnable, str(text))
                            for text in dataframe.datatable.df[column_name]
                        )
                    )
                    dataframe.datatable.df[new_column_name] = (
                ...
                self.notify(
                    "Error executing an action..." + str(e) + traceback.format_exc(),
                    severity="error",
                )
