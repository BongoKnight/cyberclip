...
class searchYetiAction(actionInterface):
    ...
    def __init__(
        self,
        parsers={},
        supportedType={"ip", "ipv6", "domain", "mail", "url"},
        complex_param={"Source": "", "Threat": [], "Campaign": [], "Tags": []},
    ):
        super().__init__(
        ...
        for obs_type in self.supportedType:
            ...
            if len(observables) > 0:
                try:
                    ...
                    headers.update(
                        {
                            "X-Api-Key": self.conf.get("api_key"),
                            "Content-Type": "application/json",
                        }
                    )
                    params = {
                        "observables": [
                            {"tags": tags, "value": o, "source": source}
                            for o in observables
                        ]
                    }
                    resp = requests.post(
                        self.conf.get("url") + "/api/observable/bulk",
                        headers=headers,
                        verify=False,
                        json=params,
                    )
